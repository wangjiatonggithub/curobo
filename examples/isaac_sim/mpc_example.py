...
def draw_points(rollouts: torch.Tensor):
    ...
    b, h, _ = cpu_rollouts.shape # 轨迹条数，每条轨迹长度，三维空间（轨迹条数可以大于1，因为用的是MPPI采样）
    point_list = []
    colors = []
    for i in range(b):
        # get list of points:
    ...
    draw.draw_points(point_list, colors, sizes) # 绘制点云
...
def main():
    ...
    robot_cfg["kinematics"]["collision_sphere_buffer"] += 0.02 # 增加机械臂碰撞球半径的安全裕度
    ...
    mpc_config = MpcSolverConfig.load_from_robot_config(
        ...
        self_collision_check=True, # 自碰撞检测，以防机器人自己碰到自己
        collision_checker_type=CollisionCheckerType.MESH,
        collision_cache={"obb": n_obstacle_cuboids, "mesh": n_obstacle_mesh},
        use_mppi=True,
        use_lbfgs=False,
        use_es=False, # 不使用进化策略优化
        store_rollouts=True, # 保留轨迹用于可视化
        step_dt=0.02,
    )

    mpc = MpcSolver(mpc_config) # 加载配置参数

    retract_cfg = mpc.rollout_fn.dynamics_model.retract_config.clone().unsqueeze(0) # 获取机械臂默认收回关节配置
    joint_names = mpc.rollout_fn.joint_names

    state = mpc.rollout_fn.compute_kinematics( # 正运动学计算末端位姿
        JointState.from_position(retract_cfg, joint_names=joint_names)
    )
    current_state = JointState.from_position(retract_cfg, joint_names=joint_names) # 当前关节状态
    retract_pose = Pose(state.ee_pos_seq, quaternion=state.ee_quat_seq) #目标末端位姿打包成Pose类型
    goal = Goal(
        current_state=current_state, # 当前关节状态
        goal_state=JointState.from_position(retract_cfg, joint_names=joint_names), # 目标关节状态
        goal_pose=retract_pose, # 目标末端位姿
    )

    goal_buffer = mpc.setup_solve_single(goal, 1) # 设置目标缓冲区，避免反复分配内存
    mpc.update_goal(goal_buffer)
    mpc_result = mpc.step(current_state, max_attempts=2) # MPC求解器预热
    ...
    while simulation_app.is_running():
        ...
        if step_index % 1000 == 0:
            ...
            mpc.world_coll_checker.load_collision_model(obstacles) # 把环境障碍物添加到MPC求解器
        ...
        cmd_state_full = mpc_result.js_action # 存储MPC求解的关节状态
        common_js_names = []
        idx_list = []
